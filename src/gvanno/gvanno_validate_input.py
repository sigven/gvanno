...
def __main__():
   ...
   ret = validate_gvanno_input(args.gvanno_dir, args.input_vcf, args.configuration_file)
   if ret != 0:
      sys.exit(-1)
...
def check_existing_vcf_info_tags(input_vcf, gvanno_directory, logger):
   ...
   gvanno_infotags_desc = gvannoutils.read_infotag_file(os.path.join(gvanno_directory,'data','gvanno_infotags.tsv'))

   vcfanno_tags = {}
   for db in ['dbsnp','docm','civic','cbmdb','dbnsfp','clinvar','uniprot','gvanno_xref']:
      vcfanno_tag_file = os.path.join(gvanno_directory,'data',str(db),str(db) + '.vcfanno.vcf_info_tags.txt')
      try:
   ...
   for e in vcf.header_iter():
      ...
      if 'ID' in header_element.keys() and 'HeaderType' in header_element.keys():
         if header_element['HeaderType'] == 'INFO':
            if header_element['ID'] in gvanno_infotags_desc.keys() or header_element['ID'] in vcfanno_tags.keys() or header_element['ID'] == 'EFFECT_PREDICTIONS':
...
def validate_gvanno_input(gvanno_directory, input_vcf, configuration_file):
   ...
   if not input_vcf == 'None':
      ...
      if valid_vcf == -1:
         return -1
      tag_check = check_existing_vcf_info_tags(input_vcf, gvanno_directory, logger)
      if tag_check == -1:
         return -1
